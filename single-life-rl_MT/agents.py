...
class SACAgent:
    def __init__(self, obs_shape=39, action_shape=4, device='cuda:0', lr=0.0001, feature_dim=256, hidden_dim=256, critic_target_tau=0.005,
                 reward_scale_factor=2, use_tb=True, from_vision=False, env_name=None):
...
class Discriminator(SACAgent):
    def __init__(self, *agent_args, env_name=None, discrim_type='state', discrim_hidden_size=128, discrim_lr=3e-4, mixup=False, q_weights=False, num_discrims=1, discrim_eps=1e-10, **agent_kwargs):
    ...
    def update_discriminator(self, pos_replay_iter, neg_replay_iter, disc_ind, val_function=None, current_val=0, current_obs=None, min_q=0, max_q=1, baseline=0, task_id = None):
        ...
        # idx = np.where((obs_pos[-7:] == task_id))
        ...
        # idx corresponds to other tasks
        # labels[idx] = 0
        ...
        if not self.q_weights:
            ...
            if baseline == 0:
                ...
                obs_b = torch.tensor(current_obs.observation).view(1,-1).cuda()
                action_b = torch.tensor(current_obs.action).view(1,-1).cuda()

                if 'sawyer' in self.env_name:
                    # obs_b = torch.Tensor(np.append(obs_b.numpy(), np.array([1,0])))
                    b = val_function(obs_b.float(), action_b.float())

                else:
                    b, _ = val_function(obs_b.float(), action_b.float())
               
                b = (b - min_q) / (max_q - min_q + 0.000001)
            else:
    ...
    def update_discriminators(self, pos_replay_iter, neg_replay_iter, val_function=None, current_val=0, current_obs=None, min_q=0, max_q=1, baseline=0, task_id = None):
        for ind, discriminator in enumerate(self.discriminators):
            metrics = self.update_discriminator(pos_replay_iter, neg_replay_iter, ind, val_function, current_val, current_obs, min_q, max_q, baseline, task_id= task_id)
        return metrics
